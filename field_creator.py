...
class Field_Creator:
    ...
    def generate_by_graph(self):
        ...
        que = deque()
        visited = np.ones((game_field_size_x,game_field_size_y))
        ...
        while len(que) != 0:
            ...
            if cur_dot[0] + 1 < game_field_size_x - 1 and visited[cur_dot[0] + 1][cur_dot[1]] == 1:
                visited[cur_dot[0] + 1][cur_dot[1]] = 0
                que.append([cur_dot[0] + 1, cur_dot[1]])
        
            if cur_dot[0] - 1 >= 1 and visited[cur_dot[0] - 1][cur_dot[1]] == 1:
                visited[cur_dot[0] - 1][cur_dot[1]] = 0
                que.append([cur_dot[0] - 1, cur_dot[1]])

            if cur_dot[1] + 1 < game_field_size_y - 1 and visited[cur_dot[0]][cur_dot[1] + 1] == 1:
                visited[cur_dot[0]][cur_dot[1] + 1] = 0
                que.append([cur_dot[0], cur_dot[1] + 1])

            if cur_dot[1] - 1 >= 1 and visited[cur_dot[0]][cur_dot[1] - 1] == 1:
                visited[cur_dot[0]][cur_dot[1] - 1] = 0
                que.append([cur_dot[0], cur_dot[1] - 1])
        ...
        for i in range(game_field_size_x):
            for j in range(game_field_size_y):
                if visited[i][j] == 1:
                    self.game_field[i * game_field_size_x + j].iswall = True
                else:
                    self.game_field[i * game_field_size_x + j].iswall = False
