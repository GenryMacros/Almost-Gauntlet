...
class Monster_Generator(pygame.sprite.Sprite):
    def __init__(self, map, pos_x, pos_y, pos_i, pos_j, lvl_matrix):
        super().__init__()
        self.x = pos_x
        self.y = pos_y
        self.map = map
    ...
    def spawn(self,walls):
        if self.cur_mobs == self.max_mobs or (pygame.time.get_ticks() - self.lastr_spawn)/1000 < self.spawn_rate:
            return None
        self.cur_mobs += 1
        newEnemy = Enemy(self.monster_pack, walls, self.rect.x, self.rect.y,self.pos_i, self.pos_j, self.lvl_matrix)
